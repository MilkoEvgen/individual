...
public class AuthRestControllerV1Test {
    ...
    @Test
    @Order(2)
    public void registerShouldThrowExceptionIfUsernameAlreadyExists() throws JsonProcessingException {
        mockWebServer.enqueue(new MockResponse()
                .addHeader("Content-Type", "application/json")
                .setBody(objectMapper.writeValueAsString(individualOutputDto)));
    ...
    @Test
    @Order(3)
    public void loginShouldReturnAuthResponseDto(){
    ...
    @Test
    @Order(4)
    public void loginShouldThrowExceptionIfLoginOrPasswordIsWrong(){
        authRequestDto.setLogin("wrong login");
        webTestClient.post().uri("/api/v1/auth/login")
                .contentType(MediaType.APPLICATION_JSON)
                .bodyValue(authRequestDto)
                .exchange()
